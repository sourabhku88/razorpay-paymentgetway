...
const Card = () => {
    const buyProduct = async () => {
        try {
            let orderApiResponse = await fetch('http://localhost:5000/order', {
                method: "POST",
                headers: {
            ...
            orderApiResponse = await orderApiResponse.json()

            let apiResponse = await fetch('http://localhost:5000/acces-key?name=sourabh&password={PASSWORD}:00', {
                method: "POST",
            });
            ...
            const options = {
                ...
                "image": imageLink,
                "order_id": orderApiResponse.result.id, //This is a sample Order ID. Pass the `id` obtained in the response of Step 1
